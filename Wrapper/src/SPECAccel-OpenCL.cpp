...
int main(int argc, char* argv[])
{
	...
	const string file_name = "spec_opencl_ida";
	const string file_size = "ref";
	const string tune = "base";

	const string applications[] = {"stencil", "lbm", "mriq", "heartwall", "cfd", "lud", "hotspot", "nw", "kmeans", "bfs", "srad", "ge", "lavamd", "bplustree"};

	for (int i = 0; i < 14; i++) {
		for(int j = 0; j< iterations; j++) {
			string command = "runspec --config=Suejb-opencl-titan.cfg --platform NVIDIA --device GPU --size="+file_size+" --tune="+tune+" --iterations=1 --noreportable --action=onlyrun --nobuild  "+applications[i];
