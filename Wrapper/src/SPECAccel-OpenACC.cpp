...
int main(int argc, char* argv[])
{
	...
	const string file_name = "spec_openacc_ida";
	const string file_size = "ref";
	const string tune = "base";

	const string applications[] = {"ostencil", "olbm", "omriq"};

	for (int i = 0; i < 3; i++) {
		for(int j = 0; j< iterations; j++) {
			string command = "runspec --config=Suejb-openacc-titan.cfg --platform NVIDIA --device GPU --size="+file_size+" --tune="+tune+" --iterations=1 --noreportable --action=onlyrun --nobuild  "+applications[i];
